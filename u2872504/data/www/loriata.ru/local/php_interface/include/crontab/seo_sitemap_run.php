<?php
$_SERVER['DOCUMENT_ROOT'] = '/var/www/u2872504/data/www/loriata.ru';
...
use Imedia\Main\Helpers\Sitemap\Runtime as SitemapRuntimeCustom;

Loc::loadMessages($_SERVER['DOCUMENT_ROOT'] . '/bitrix/modules/seo/admin/seo_sitemap.php');

Main\Loader::includeModule('imedia.main');
        ...
        if($v == $arValueSteps['init']) {
            ...
            if(Main\Loader::includeModule('iblock')) {
                ...
                if(count($arIBlockList) > 0) {
                    $arIBlocks = [];
                    $dbIBlock  = CIBlock::GetList([], array('ID' => array_keys($arIBlockList)));
                        ...
                        if($dbTopicResult == null) {
                            $dbTopicResult = CForumTopic::GetList(
                                ...
                                array_merge(
                                    ...
                                    ($NS['FORUM_CURRENT_TOPIC'] > 0) ? [">ID" => $NS["FORUM_CURRENT_TOPIC"]] : []
                                ),
                                false,
                                0,
                                ['nTopCount' => 100]
