...
if(strlen($request->get('ORDER_ID')) > 0){
    ...
    if(empty($arResult['GRID']['ROWS'])){
        ...
        $currencyFormat = \CCurrencyLang::GetCurrencyFormat($arResult['BASE_LANG_CURRENCY'], LANGUAGE_ID);

        $deliveryPickupIds = [];

        foreach(\CDeliverySDEK::getDeliveryId('pickup') as $id){
            $deliveryPickupIds[] = (int) $id;
        }

        foreach(\CDeliverySDEK::getDeliveryId('postamat') as $id){
            $deliveryPickupIds[] = (int) $id;
        }

        $arDeliveryPickup = Delivery\Services\Table::getList(
            [
                'select' => ['ID'],
                'filter' => ['=XML_ID' => 'PICKUP'],
                'limit' => 1
            ]
        )->fetch();
        if($arDeliveryPickup){
            $deliveryPickupIds[] = $arDeliveryPickup['ID'];
        }
        ...
        $params = [
            ...
            'pathAjax' => $component->getPath().'/ajax.php',
            'currencyFormat' => [
            ...
            'isAuthorized' => (bool) $currentUser->getId(),
            'userId' => (int) $currentUser->getId(),
            'propertyPickupAddress' => Option::get('ipol.sdek', 'address'),
            'deliveryPickupIds' => $deliveryPickupIds
        ];

        ?>
        <order :params='<?=Json::encode($params)?>'>
            <template slot="empty">
                <?php include 'empty.php'?>
            </template>
        </order>
