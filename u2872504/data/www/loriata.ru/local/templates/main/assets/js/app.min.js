    ...
    $(document).ready(function() {
        ...
        window.NodeList && !NodeList.prototype.forEach && (NodeList.prototype.forEach = function(callback, thisArg) {
                ...
                init: function() {
                    $(".aside-filters__item-drop").on("click", function() {
                        $(this).toggleClass("aside-filters__item-drop--active"),
                            ...
                            $(this).hasClass("aside-filters__item-drop--active") ? $(this).attr("aria-label", "Свернуть") : $(this).attr("aria-label", "Развернуть")
                    })
                }
            },
            app.blog = {
                    ...
                    $(".catalog-card__favorite").on("click", function() {
                        ...
                        $(".catalog__view-btn").on("click", function() {
                            $(this).addClass("catalog__view-btn--active"),
                                ...
                                $(".col-btn").hasClass("catalog__view-btn--active") ? $(".catalog__cards").addClass("catalog__cards--two-cols") : $(".catalog__cards").removeClass("catalog__cards--two-cols"),
                                $(".row-btn").hasClass("catalog__view-btn--active") ? $(".catalog__cards").addClass("catalog__cards--row") : $(".catalog__cards").removeClass("catalog__cards--row")
                        }),
                    $(window).width() < 768 && ($(".catalog__cards").addClass("catalog__cards--two-cols"),
            ...
            app.dropBasket = {
                ...
                description: "your script description",
                init: function() {
                    0 == $(".drop-basket__products").length ? $(".drop-basket__empty").show() : $(".drop-basket__empty").hide()
                }
            },
            app.faq = {
                name: "faq",
            ...
            app.pageup = {
                ...
                init: function() {
                    ...
                    window.addEventListener("scroll", function() {
                        window.scrollY > window.innerHeight ? pageUp.classList.add("active") : pageUp.classList.remove("active"),
                            window.scrollY >= footerOffset - footerHeight - page.children[page.children.length - 1].getBoundingClientRect().height ? pageUp.classList.add("in-footer") : pageUp.classList.remove("in-footer")
                    })
                }
            },
            app.productCard = {
                ...
                description: "your script description",
                init: function() {
                    $(".minus-btn").on("click", function($input) {
                        $input.preventDefault();
                        for (var $input = $(this).next(), value = parseInt($input.val()); 0 < value; ) {
                            --value;
                            break
                        }
                        $input.val(value)
                    }),
                        $(".plus-btn").on("click", function(value) {
                        ...
                        $(".product-color__item .radio-box").on("change", function() {
                            $(this).is(":checked") && ($(this).next().addClass("radio-style--checked"),
                                $(this).parent().siblings().find("span.radio-style").removeClass("radio-style--checked"))
                        }),
                        $(".scroll-link").on("click", function() {
                            return $(".product-card__tab").removeClass("tab--active"),
                ...
                init: function() {
                    $(".product-details__hide-btn").on("click", function() {
                        $(this).toggleClass("product-details__hide-btn--active"),
                            $(this).parent().next().slideToggle("300"),
                            $(this).hasClass("product-details__hide-btn--active") ? $(this).text("скрыть") : $(this).text("раскрыть")
                    }),
            ...
            app.productSizes = {
                ...
                init: function() {
                    $(".product-sizes__item").on("click", function() {
                        $(this).find("input[type=radio]").prop("disabled") && $.fancybox.open({
                ...
                init: function() {
                    $(".search-form__input").on("input", function() {
                        ...
                        $(".search-form__input").on("input change", function() {
                            "" === $(this).val() ? $(this).next().fadeOut("slow") : $(this).next().fadeIn("slow")
                        }),
                        $(".search-form__input").on("focus", function() {
                            "" !== $(this).val() && $(this).next().fadeIn("slow")
                        }),
            ...
            app.validation = {
                ...
                description: "your script description",
                init: function() {
                    function validateForms(_messages) {
                        $(_messages).validate({
                            rules: {
                                name: "required",
                                surname: "required",
                            ...
                            messages: (_defineProperty(_messages = {
                        ...
                        $.fn.setCursorPosition = function(pos) {
                            var range;
                            $(this).get(0).setSelectionRange ? $(this).get(0).setSelectionRange(pos, pos) : $(this).get(0).createTextRange && ((range = $(this).get(0).createTextRange()).collapse(!0),
            ...
            (new WOW).init(),
            $("div.catalog-dropdown__title span").on("click", function() {
                var $content = $(this)
        ...
        $("main").hasClass("drop-basket-page") && ($("body").addClass("_lock"),
            $(".overlay").show("slow"),
            ...
            $(".tab:not(.profile-page__tab):not(.tab--unhandled)").on("click", function(e) {
                e.preventDefault(),
                ...
                var parent = $(this).closest('.tabs-parent')[0];
                var tabId = $(this).attr('href').slice(1);

                document.dispatchEvent(new CustomEvent('tabAfterChange', {
                    detail: {
                        id: tabId,
                        parent: parent
                    }
