...
$APPLICATION->SetPageProperty('classes--page', 'news-page');

$request = Context::getCurrent()->getRequest();
$selectedTags = ($request->getQuery('tags'))
    ? explode(',', $request->getQuery('tags'))
    : null;
...
if(!empty($selectedTags)){
    $tagsFilter = ['LOGIC' => 'AND'];
    foreach($selectedTags as $key => $tag){
        $tag = Text::mb_ucfirst($tag);
        $selectedTags[$key] = $tag;
        $tagsFilter[] = ['?TAGS' => $tag];
    }

    $GLOBALS[$arParams['FILTER_NAME']][] = $tagsFilter;
}
